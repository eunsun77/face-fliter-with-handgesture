...
spyderman_preview = cv2.imread("filters/spyderman_preview.png")
ironman_preview = cv2.imread("filters/ironman_preview.png")
color_cat_preview = cv2.imread("filters/color_cat_preview.png")
...
filters_config = {
    ...
    'spyderman':
        [{'path': "filters/spyderman.png",
          ...
          'morph': True, 'animated': False, 'has_alpha': True}],
    'Rudolf':
        [{'path': "filters/Rudolf_horn.png",
          'anno_path': "filters/Rudolf_horn_annotations.csv",
          'morph': False, 'animated': False, 'has_alpha': True},
         {'path': "filters/Rudolf_nose.png",
          'anno_path': "filters/Rudolf_nose_annotations.csv",
          'morph': False, 'animated': False, 'has_alpha': True}],
    'ironman':
        [{'path': "filters/ironman.png",
          'anno_path': "filters/ironman_annotations.csv",
          'morph': True, 'animated': False, 'has_alpha': True}],
    'color_cat':
        [{'path': "filters/color_cat.png",
          'anno_path': "filters/color_cat_annotations.csv",
          'morph': True, 'animated': False, 'has_alpha': True}]
}
...
while True:
    ...
    if not ret:
        ...
        if number == 0:
            ...
            filters, multi_filter_runtime = load_filter("spyderman")
                
        elif number == 4:
            filters, multi_filter_runtime = load_filter("ironman")
                
        elif number == 5:
            filters, multi_filter_runtime = load_filter("color_cat")
    ...
    spyderman_preview_resized = cv2.resize(spyderman_preview, (150, 150))
    spyderman_preview_resized = cv2.cvtColor(spyderman_preview_resized, cv2.COLOR_BGR2RGB)
    
    hand_4_resized = cv2.resize(hand_4, (100, 150))
    ironman_preview_resized = cv2.resize(ironman_preview, (150, 150))
    ironman_preview_resized = cv2.cvtColor(ironman_preview_resized, cv2.COLOR_BGR2RGB)
    
    hand_5_resized = cv2.resize(hand_5, (100, 150))
    color_cat_preview_resized = cv2.resize(color_cat_preview, (150, 150))
    color_cat_preview_resized = cv2.cvtColor(color_cat_preview_resized, cv2.COLOR_BGR2RGB)
    
    # 미리보기 이미지를 화면 우측에 넣기
    frame[0:1080, 1650:1920] = black_bg_resized
    
    frame[65:215, 1660:1760] = hand_1_resized
    frame[65:215, 1760:1910] = cat_preview_resized
    
    frame[265:415, 1660:1760] = hand_2_resized
    frame[265:415, 1760:1910] = dog_preview_resized
    
    frame[465:615, 1660:1760] = hand_3_resized
    frame[465:615, 1760:1910] = spyderman_preview_resized
    
    frame[665:815, 1660:1760] = hand_4_resized
    frame[665:815, 1760:1910] = ironman_preview_resized
    
    frame[865:1015, 1660:1760] = hand_5_resized
    frame[865:1015, 1760:1910] = color_cat_preview_resized
    ...
    if(number == 0):
        ...
        print(f"제스처: {number}, 필터: Spyderman")
        
    elif(number == 4):
        print(f"제스처: {number}, 필터: Ironman")
        
    elif(number == 5):
        print(f"제스처: {number}, 필터: Color Cat")
